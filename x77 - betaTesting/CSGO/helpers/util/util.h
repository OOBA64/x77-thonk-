...
namespace u
{
	...
	/*inline void transform_screen(const vector& in, vector& out) {
		static ptrdiff_t ptrViewMatrix;
		if (!ptrViewMatrix)
		{//                                                          
			ptrViewMatrix = static_cast<ptrdiff_t>(u::find_pattern("client_panorama.dll", "0F 10 05 ? ? ? ? 8D 85 ? ? ? ? B9"));
			ptrViewMatrix += 0x3;
			ptrViewMatrix = *reinterpret_cast<uintptr_t*>(ptrViewMatrix);
			ptrViewMatrix += 176;
		}
		const matrix3x4_t& worldToScreen = i::engineclient->world_to_screen_matrix(); // matrix
